  ...
  computed:{
    tableData:function(){
      ...
      for (var k = 0,len3=tData.length; k < len3; k++) {
          ...
          for (var k1 = k+1; k1 < len3; k1++) {
              ...
              rowspan*=kSpecLen;
              console.log("kSpecLen：",kSpecLen,"rowspan:",rowspan)
          }
    ...
    tableList:function(){
       ...
       for (var i = 0; i < rows; i++) {
          ...
          //构建固定项(price,number)
          console.log(this.verdictChange)
          if(this.verdictChange){
              listItem['price'] = "";
              listItem['number'] = "";
          }
          tList.push(listItem);
  ...
  methods: {
    addItem:function(){
      for(let i = 0;i<this.attrs.length;i++){
          if(this.attrs[i].pName==""){
              this.$message.error('请填写规格名')
              return false
          }
          if(this.attrs[i].spec.length==0){
              this.$message.error('有未添加规格值')
              return false
          }
          if(this.attrs[i].spec.length!=0){
              for(let j=0;j<this.attrs[i].spec.length;j++){
                  if(this.attrs[i].spec[j].cName==''){
                      this.$message.error('有未添加规格值')
                      return false
                  }
              }
      ...
      // }
      this.attrs.push(obj);
      this.verdictChange = true
    },
    toDelete:function(index){
        this.attrs.splice(index,1);
    },
    toConfirm:function(){
        console.log(this.tableList,this.tableData,this.rows);
